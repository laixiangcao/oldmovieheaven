...
//        mMvoietype = getArguments().getInt("mvoietype", -1);
        mMvoietype = (int) getArguments().getSerializable("mvoietype");
        moviedata = (ArrayList<MovieDao>) PrefUtils.readObject("MovieCache"+mMvoietype);
        if (moviedata != null) {
    ...
    private void initNetData() {
        NetWorkApi.getMovieList(mMvoietype, 1, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                moviedata = ParseData.parseClassifyMovie(content);
                if (moviedata.size() != 0 && moviedata != null) {
                    PrefUtils.saveObject("MovieCache"+mMvoietype, moviedata);
                }
                loadDataComplete(check(moviedata));
            }
    ...
    protected void requestLoadMoreData() {
        ...
        NetWorkApi.getMovieList(mMvoietype, ++currentPage, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                String content = null;
                try {
                ...
                ArrayList<MovieDao> moreData = ParseData.parseClassifyMovie(content);
                if (moreData != null) {
                    if (moreData.size() == 0) {
                        setFooterType(TYPE_NO_MORE);//没有更多数据
    ...
    public void onItemClick(AdapterView parent, View view, int position, long id) {
        ...
        bundle.putString("movieType", "classify");
    ...
    protected void requestRefreshData() {
        NetWorkApi.getMovieList(mMvoietype, 1, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                pages = ParseData.getCategoryPages(content);
                ArrayList<MovieDao> refreshDataList = ParseData.parseClassifyMovie(content);
                if (refreshDataList.size() != 0 && refreshDataList != null) {
                    PrefUtils.saveObject("MovieCache"+mMvoietype, refreshDataList);
